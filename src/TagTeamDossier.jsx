...
import TagTeamDossier from './TagTeamDossier';
...
export default function TagTeamsView() {
  ...
  const deleteTeam = async (id, e) => {
    e.stopPropagation(); // Ať to při mazání neotvírá ten detail
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {teams.map(team => (
          <motion.div 
            ...
            className="bg-dark p-6 rounded-2xl border border-gray-800 flex justify-between shadow-lg cursor-pointer hover:border-accent transition-colors"
          >
            <div>
              <h3 className="text-2xl font-bold text-accent">{team.name}</h3>
              <p className="text-sm text-gray-500 mt-2 font-bold uppercase">Click for Dossier</p>
            </div>
            <button onClick={(e) => deleteTeam(team.id, e)} className="text-gray-600 hover:text-red-500 transition-colors z-10">
              <Trash2 size={24} />
            </button>
          </motion.div>
        ))}
